...
def load_existing_videos(filename="videos.json"):
    ...
    try:
        ...
        if not isinstance(data, list):
            ...
            return [], set()
        # Build set of existing video_ids for fast lookup
...
def merge_and_save(new_videos, filename="videos.json"):
    """Merge new videos with existing ones, avoiding duplicates, sort, and save."""
    existing_list, existing_ids = load_existing_videos(filename)
    ...
    existing_list.sort(key=lambda v: v["published"], reverse=True)
    
    # Write back
    with open(filename, "w", encoding="utf-8") as f:
        json.dump(existing_list, f, indent=2, ensure_ascii=False)
    
    print(f"\nAdded {added} new videos. Total now: {len(existing_list)}")
...
def main():
    ...
    merge_and_save(all_new_videos, "videos.json")
    
    print("File videos.json updated successfully.")
